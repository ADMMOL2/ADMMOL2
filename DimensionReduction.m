...
%L2data = load('data/L2coefficient.txt');
...
OL12data = sort(abs(OL12data),'descend');
len = length(OL12data);
newp = zeros(1,len);
i = 1;
for i = 1:len  % q=4  
  if OL12data(i) >= (i/len)*0.4
  res = sprintf('%f   %f      %f \n',i,OL12data(i),(i/len)*0.4);
  newp(i) = OL12data(i);
  else
      newp(i) = 0;
  end
...
hold on
plot(OL12data,'m--','MarkerSize', 10, 'LineWidth', 3);
hold on
