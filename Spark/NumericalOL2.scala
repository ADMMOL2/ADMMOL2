...
object NumericalOL2 {
  ...
  def main(args: Array[String]) {
    ...
    rng.setSeed(0);
    val m = 1500;       // number of examples (rows)
    val n = 5000;       // number of features (Column)
    ...
    /********val x0 = BSM.rand(n,1,Rand.gaussian);//SparseMatrix.sprandn(n,1,p,rng); //x0 = initial matrix*/
    val x0 = BDV.rand[Double](n,spDensdity)
    var A:BDM[Double] =  BDM.rand(m,n);
    val sumofSqrt = diag(1.0:/(sum(A:^2.0,Axis._0):^0.5).t);
    A = A*sumofSqrt;
    val b:BDV[Double] = A*x0 + sqrt(0.001)*BDV.rand[Double](m);
